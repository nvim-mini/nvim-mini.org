...
- Helper for creating child Neovim process which is designed to be used in
  ...
  [MiniTest.new\_child\_neovim()](mini-test.qmd#minitest.new_child_neovim) and [MiniTest.expect.reference\_screenshot()](mini-test.qmd#minitest.expect.reference_screenshot).


- Hierarchical organization of tests with custom hooks, parametrization,
  and user data. See [MiniTest.new\_set()](mini-test.qmd#minitest.new_set).
...
- Test case filtering. There are predefined wrappers for testing a file
  ([MiniTest.run\_file()](mini-test.qmd#minitest.run_file)) and case at a location like current cursor position
  ([MiniTest.run\_at\_location()](mini-test.qmd#minitest.run_at_location)).


- Customizable reporter of output results. There are two predefined ones:

    - [MiniTest.gen\_reporter.buffer()](mini-test.qmd#minitest.gen_reporter.buffer) for interactive usage.

    - [MiniTest.gen\_reporter.stdout()](mini-test.qmd#minitest.gen_reporter.stdout) for headless Neovim.
...
- "Overly specific" expectations. Tests for (no) equality and (absence of)
  ...
  complexity. Use [MiniTest.new\_expectation()](mini-test.qmd#minitest.new_expectation) to create custom ones.
...
- Code of this plugin's tests. Consider it to be an example of intended
  way to use [mini.test](mini-test.qmd#mini.test) for test organization and creation.
...
- Write test actions as callable entries of test set. Use child process
  inside test actions (see [MiniTest.new\_child\_neovim()](mini-test.qmd#minitest.new_child_neovim)) and builtin
...
- Testing infrastructure from
  [nvim-lua/plenary.nvim](https://github.com/nvim-lua/plenary.nvim):
    ...
    - Executes each file in separate headless Neovim process with customizable
      `init.vim` file. While [mini.test](mini-test.qmd#mini.test) executes everything in current
      Neovim process encouraging writing tests with help of manually
      managed child Neovim process (see [MiniTest.new\_child\_neovim()](mini-test.qmd#minitest.new_child_neovim)).

    - Tests are expected to be written with embedded simplified versions of
      `lunarmodules/busted` and `lunarmodules/luassert`. While [mini.test](mini-test.qmd#mini.test)
      uses concepts of test set (see [MiniTest.new\_set()](mini-test.qmd#minitest.new_set)) and test case
    ...
    - Has single way of reporting progress (shows result after every case
      without summary). While [mini.test](mini-test.qmd#mini.test) can have customized reporters
      with defaults for interactive and headless usage (provide more
      compact and user-friendly summaries).

    - Allows parallel execution, while [mini.test](mini-test.qmd#mini.test) does not.

    - Allows making mocks, stubs, and spies, while [mini.test](mini-test.qmd#mini.test) does not in
      favor of manually overwriting functionality in child Neovim process.

Although [mini.test](mini-test.qmd#mini.test) supports emulation of "busted style" testing, it will
...
- "Busted" syntax doesn't support full capabilities offered by [mini.test](mini-test.qmd#mini.test).
...
Some hints for converting from `plenary.nvim` tests to [mini.test](mini-test.qmd#mini.test):

- Rename files from `***_spec.lua` to `test_***.lua` and put them in
  "tests" directory.

- Replace `assert` calls with [mini.test](mini-test.qmd#mini.test) expectations. See [MiniTest.expect](mini-test.qmd#minitest.expect).

- Create main test set `T = MiniTest.new_set()` and eventually return it.

- Make new sets ([MiniTest.new\_set()](mini-test.qmd#minitest.new_set)) from `describe` blocks. Convert
...
<a name="minitest-hl-groups" href="mini-test.qmd#minitest-hl-groups"><b>MiniTest-hl-groups</b></a>
...
### Test case {#minitest-test-case .help-syntax-right-anchor}

Test case

An item of sequential test organization, as opposed to hierarchical with
test set (see [MiniTest.new\_set()](mini-test.qmd#minitest.new_set)). It is created as result of test
...
option in [MiniTest.new\_set()](mini-test.qmd#minitest.new_set)).
...
- Source each file from array output of `opts.find_files`. It should output
  a test set (see [MiniTest.new\_set()](mini-test.qmd#minitest.new_set)) or `nil` (if "busted" style is used;
  ...
  element do: ...". Details:
    ...
    - If element is a test set, process it in similar, recursive fashion.
      The "current nested" information is expanded:
        ...
        - `data` is extended via [vim.tbl\_deep\_extend()](https://neovim.io/doc/user/helptag.html?tag=vim.tbl_deep_extend\(\)).
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Options controlling case collection. Possible fields:
  ...
  - <span class="help-syntax-keys">\<reporter\></span> - table with possible callable fields `start`, `update`,
    `finish`. Default: [MiniTest.gen\_reporter.buffer()](mini-test.qmd#minitest.gen_reporter.buffer) in interactive
    usage and [MiniTest.gen\_reporter.stdout()](mini-test.qmd#minitest.gen_reporter.stdout) in headless usage.
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Options with fields:
  ...
  - <span class="help-syntax-keys">\<close_all_child_neovim\></span> - whether to close all child neovim processes
    created with [MiniTest.new\_child\_neovim()](mini-test.qmd#minitest.new_child_neovim). Default: `true`.
...
Mostly designed to be used within [mini.test](mini-test.qmd#mini.test) framework.
...
Equality is tested via [vim.deep\_equal()](https://neovim.io/doc/user/helptag.html?tag=vim.deep_equal\(\)). It also tries to compute more
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Options. Possible fields:
  ...
  - <span class="help-syntax-keys">\<fail_reason\></span> `(string|function)` - reason for failing expectation.
...
Equality is tested via [vim.deep\_equal()](https://neovim.io/doc/user/helptag.html?tag=vim.deep_equal\(\)).
...
<span class="help-syntax-special">{f}</span> `(function)` Function to be tested for raising error.
...
<span class="help-syntax-special">{f}</span> `(function)` Function to be tested for not raising error.
...
<span class="help-syntax-special">{screenshot}</span> `(table|nil)` Array with screenshot information. Usually an output
  of `child.get_screenshot()` (see [MiniTest-child-neovim-get\_screenshot()](mini-test.qmd#minitest-child-neovim-get_screenshot)).
...
- `<Esc>` - stop test execution if executing (see [MiniTest.is\_executing()](mini-test.qmd#minitest.is_executing)
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Table with options. Used fields:
  ...
  - <span class="help-syntax-keys">\<window\></span> - definition of window to open. Can take one of the forms:
      ...
      - Table. Used as `config` argument in [nvim\_open\_win()](https://neovim.io/doc/user/helptag.html?tag=nvim_open_win\(\)).
...
It has same general idea as [MiniTest.gen\_reporter.buffer()](mini-test.qmd#minitest.gen_reporter.buffer) with slightly
...
This creates an object designed to be a fundamental piece of [mini.test](mini-test.qmd#mini.test)
...
### Child neovim {#minitest-child-neovim .help-syntax-right-anchor}
...
<span class="help-syntax-special">{type_keys}</span> `(function)` Emulate typing keys.
  See [MiniTest-child-neovim-type\_keys()](mini-test.qmd#minitest-child-neovim-type_keys). Doesn't check for blocked state.


<span class="help-syntax-special">{cmd}</span> `(function)` Execute Vimscript code from a string.
  A wrapper for [nvim\_exec()](https://neovim.io/doc/user/helptag.html?tag=nvim_exec\(\)) without capturing output.

<span class="help-syntax-special">{cmd_capture}</span> `(function)` Execute Vimscript code from a string and
  capture output. A wrapper for [nvim\_exec()](https://neovim.io/doc/user/helptag.html?tag=nvim_exec\(\)) with capturing output.


<span class="help-syntax-special">{lua}</span> `(function)` Execute Lua code. A wrapper for [nvim\_exec\_lua()](https://neovim.io/doc/user/helptag.html?tag=nvim_exec_lua\(\)).
...
<span class="help-syntax-special">{lua_get}</span> `(function)` Execute Lua code and return result. A wrapper
  for [nvim\_exec\_lua()](https://neovim.io/doc/user/helptag.html?tag=nvim_exec_lua\(\)) but prepends string code with `return`.
...
### Child neovim start() {#minitest-child-neovim-start .help-syntax-right-anchor}
...
### Child neovim type_keys() {#minitest-child-neovim-type_keys .help-syntax-right-anchor}

<p align="center">`child.type_keys`(<span class="help-syntax-special">{wait}</span>, <span class="help-syntax-special">{...}</span>)</p>

Basically a wrapper for [nvim\_input()](https://neovim.io/doc/user/helptag.html?tag=nvim_input\(\)) applied inside child process.
Differences:
...
<span class="help-syntax-special">{...}</span> `(string|table<number,string>)` Separate entries for [nvim\_input()](https://neovim.io/doc/user/helptag.html?tag=nvim_input\(\)),
...
### Child neovim get_screenshot() {#minitest-child-neovim-get_screenshot .help-syntax-right-anchor}
...
cell (row from 1 to ['lines'](https://neovim.io/doc/user/helptag.html?tag='lines'), column from 1 to ['columns'](https://neovim.io/doc/user/helptag.html?tag='columns')).
