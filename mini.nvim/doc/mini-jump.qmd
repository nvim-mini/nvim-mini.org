...
#### Highlight groups {#minijump-highlightgroups}
...
a standalone mapping or part of [`MiniKeymap.map_multistep()`](mini-keymap.qmd#minikeymap.map_multistep). For example:
...
<span class="help-syntax-special">{jumping}</span> `(boolean)` Whether module is currently in "jumping mode": usage of
  [`MiniJump.smart_jump()`](mini-jump.qmd#minijump.smart_jump) and all mappings won't require target.
...
### smart_jump() {#minijump.smart_jump}
...
### stop_jumping() {#minijump.stop_jumping}
