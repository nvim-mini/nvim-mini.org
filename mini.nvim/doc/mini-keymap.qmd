...
- Map keys to perform configurable multi-step actions: if condition for step
  ...
  "smart" keys (like "smart tab"). See [`MiniKeymap.map_multistep()`](mini-keymap.qmd#minikeymap.map_multistep).
...
- Map keys as "combo": each key acts immediately plus execute extra action if
  ...
  See [`MiniKeymap.map_combo()`](mini-keymap.qmd#minikeymap.map_combo). Some of the common use cases include:
...
#### Comparisons {#minikeymap-comparisons}



- [max397574/better-escape.nvim](https://github.com/max397574/better-escape.nvim):

    - Mostly similar to [`MiniKeymap.map_combo()`](mini-keymap.qmd#minikeymap.map_combo) with a different approach
...
- [abecodes/tabout.nvim](https://github.com/abecodes/tabout.nvim):
    ...
    - Similar general idea as in `'jump_{after,before}_tsnode'` steps
      of [`MiniKeymap.map_multistep()`](mini-keymap.qmd#minikeymap.map_multistep).
...
#### Disabling {#minikeymap-disabling}
...
### Examples {#minikeymap-examples}

#### Multi-step {#minikeymap-examples-multi-step}


See [`MiniKeymap.map_multistep()`](mini-keymap.qmd#minikeymap.map_multistep) for a general description of how multi-step
...
See [`MiniKeymap.map_combo()`](mini-keymap.qmd#minikeymap.map_combo) for a general description of what is a combo and
...
### map_multistep() {#minikeymap.map_multistep}
...
### gen_step.search_pattern() {#minikeymap.gen_step.search_pattern}
...
Built-in [`MiniKeymap.map_multistep()`](mini-keymap.qmd#minikeymap.map_multistep) steps "jump_after_close" and
...
### map_combo() {#minikeymap.map_combo}
...
- IMPORTANT! Combo is not a regular mapping but a separate key tracking
  with [`vim.on_key()`](https://neovim.io/doc/user/helptag.html?tag=vim.on_key\(\)). This is important as combos will not be visible and
...
- String action is executed with [`nvim_input()`](https://neovim.io/doc/user/helptag.html?tag=nvim_input\(\)), i.e. emulated keys will
...
- Neovim>=0.11 is recommended due to [`vim.on_key()`](https://neovim.io/doc/user/helptag.html?tag=vim.on_key\(\)) improvement to allow
...
<span class="help-syntax-special">{action}</span> `(string|function)` Action to perform after key sequence is detected.
  If string, treated as keys and emulated with [`nvim_input()`](https://neovim.io/doc/user/helptag.html?tag=nvim_input\(\)).
