...
<a name="mini.surround" href="mini-surround.qmd#mini.surround"><b>mini.surround</b></a> Surround actions
...
(see [MiniSurround-vim-surround-config](mini-surround.qmd#minisurround-vim-surround-config)).

Features:

- Actions (text editing actions are dot-repeatable out of the box and
  respect [[count]](https://neovim.io/doc/user/helptag.html?tag=[count])) with configurable mappings:
...
- Surrounding is identified by a single character as both "input" (in
  ...
  `add` and `replace` end):
    ...
    - All other single character identifiers (supported by [getcharstr()](https://neovim.io/doc/user/helptag.html?tag=getcharstr\(\)))
...
- Configurable search methods to find not only covering but possibly next,
  previous, or nearest surrounding. See more in [MiniSurround.config](mini-surround.qmd#minisurround.config).
...
- All actions involving finding surrounding (delete, replace, find,
  ...
  previous/last. See more in [MiniSurround.config](mini-surround.qmd#minisurround.config).
...
See [MiniSurround.config](mini-surround.qmd#minisurround.config) for `config` structure and default values.

You can override runtime config settings locally to buffer inside
`vim.b.minisurround_config` which should have same structure as
`MiniSurround.config`. See [mini.nvim-buffer-local-config](mini-nvim.qmd#mini.nvim-buffer-local-config) for more details.

To stop module from showing non-error feedback, set `config.silent = true`.

#### Example usage {#module-exampleusage}
...
- [tpope/vim-surround](https://github.com/tpope/vim-surround):
    ...
    - `vim-surround` has completely different, with other focus set of
      default mappings, while [mini.surround](mini-surround.qmd#mini.surround) has a more coherent set.

    - [mini.surround](mini-surround.qmd#mini.surround) supports dot-repeat, customized search path (see
      [MiniSurround.config](mini-surround.qmd#minisurround.config)), customized specifications (see
      [MiniSurround-surrounding-specification](mini-surround.qmd#minisurround-surrounding-specification)) allowing usage of tree-sitter
      queries (see [MiniSurround.gen\_spec.input.treesitter()](mini-surround.qmd#minisurround.gen_spec.input.treesitter)),
...
- [kylechui/nvim-surround](https://github.com/kylechui/nvim-surround):
    ...
    - `nvim-surround` is designed after `tpope/vim-surround` with same
      default mappings and logic, while [mini.surround](mini-surround.qmd#mini.surround) has mappings
      similar to `machakann/vim-sandwich`.

    - [mini.surround](mini-surround.qmd#mini.surround) has more flexible customization of input surrounding
      (with composed patterns, region pair(s), search methods).

    - [mini.surround](mini-surround.qmd#mini.surround) supports [[count]](https://neovim.io/doc/user/helptag.html?tag=[count]) in both input and output
      surrounding (see [MiniSurround-count-with-actions](mini-surround.qmd#minisurround-count-with-actions)) while
      `nvim-surround` doesn't.

    - [mini.surround](mini-surround.qmd#mini.surround) supports "last"/"next" extended mappings.

- [mini.ai](mini-ai.qmd#mini.ai):

    - Both use similar logic for finding target: textobject in [mini.ai](mini-ai.qmd#mini.ai)
      and surrounding pair in [mini.surround](mini-surround.qmd#mini.surround). While [mini.ai](mini-ai.qmd#mini.ai) uses
      extraction pattern for separate `a` and `i` textobjects,
      [mini.surround](mini-surround.qmd#mini.surround) uses it to select left and right surroundings
      (basically a difference between `a` and `i` textobjects).

    - Some builtin specifications are slightly different:

        - Quotes in [mini.ai](mini-ai.qmd#mini.ai) are balanced, in [mini.surround](mini-surround.qmd#mini.surround) they are not.

        - The [mini.surround](mini-surround.qmd#mini.surround) doesn't have argument surrounding.

        - Default behavior in [mini.ai](mini-ai.qmd#mini.ai) selects one of the edges into `a`
          textobject, while [mini.surround](mini-surround.qmd#mini.surround) - both.

#### Highlight groups {#module-highlightgroups}


<a name="minisurround-hl-groups" href="mini-surround.qmd#minisurround-hl-groups"><b>MiniSurround-hl-groups</b></a>
...
### Builtin surroundings {#minisurround-builtin-surroundings .help-syntax-right-anchor}
...
- `Key` represents the surrounding identifier: single character which should
  be typed after action mappings (see "Mappings" in [MiniSurround.config](mini-surround.qmd#minisurround.config)).
...
- Default surrounding is activated for all characters which are not
  configured surrounding identifiers. Notes:
    ...
    - Due to special handling of underlying `x.-x` Lua pattern
      (see [MiniSurround-search-algorithm](mini-surround.qmd#minisurround-search-algorithm)), it doesn't really support
...
### Glossary {#minisurround-glossary .help-syntax-right-anchor}

Note: this is similar to [MiniAi-glossary](mini-ai.qmd#miniai-glossary).

#### REGION {#minisurround-glossary-region}
...
inclusive start and end positions (<span class="help-syntax-keys">\<to\></span> might be `nil` to describe empty
region). Each position is also a table with line <span class="help-syntax-keys">\<line\></span> and column <span class="help-syntax-keys">\<col\></span>
(both start at 1). Examples:
...
#### REGION PAIR {#minisurround-glossary-regionpair}


Table representing regions for left and right surroundings. Fields: <span class="help-syntax-keys">\<left\></span>
and <span class="help-syntax-keys">\<right\></span> with regions. Example:
...
#### PATTERN {#minisurround-glossary-pattern}


String describing Lua pattern.

#### SPAN {#minisurround-glossary-span}
...
#### NESTED PATTERN {#minisurround-glossary-nestedpattern}


Array of patterns aimed to describe nested spans.

#### SPAN MATCHES NESTED PATTERN {#minisurround-glossary-spanmatchesnestedpattern}
...
#### COMPOSED PATTERN {#minisurround-glossary-composedpattern}
...
#### SPAN MATCHES COMPOSED PATTERN {#minisurround-glossary-spanmatchescomposedpattern}
...
### Surrounding specification {#minisurround-surrounding-specification .help-syntax-right-anchor}

Surround specification is a table with keys:

- <span class="help-syntax-keys">\<input\></span> - defines how to find and extract surrounding for "input"
  operations (like `delete`). See more in "Input surrounding" section.

- <span class="help-syntax-keys">\<output\></span> - defines what to add on left and right for "output" operations
...
#### Input surrounding {#minisurround-surrounding-specification-inputsurrounding}


Specification for input surrounding has a structure of composed pattern
(see [MiniSurround-glossary](mini-surround.qmd#minisurround-glossary)) with two differences:
  ...
  complexity and computations):
    ...
    - If specification itself is a callable, it will be called without
      arguments and should return one of:
        ...
        - Single region pair (see [MiniSurround-glossary](mini-surround.qmd#minisurround-glossary)). Useful to allow
        ...
        - Array of region pairs. Useful for incorporating other instruments,
          like treesitter (see [MiniSurround.gen\_spec.input.treesitter()](mini-surround.qmd#minisurround.gen_spec.input.treesitter)). The
...
See [MiniSurround.gen\_spec](mini-surround.qmd#minisurround.gen_spec) for function wrappers to create commonly used
surrounding specifications.

#### Output surrounding {#minisurround-surrounding-specification-outputsurrounding}


Specification for output can be either a table with <span class="help-syntax-keys">\<left\></span> and <span class="help-syntax-keys">\<right\></span> fields,
...
### Count with actions {#minisurround-count-with-actions .help-syntax-right-anchor}

[[count]](https://neovim.io/doc/user/helptag.html?tag=[count]) is supported by all actions in the following ways:
...
### Search algorithm {#minisurround-search-algorithm .help-syntax-right-anchor}
...
- Input surrounding specification is constructed based on surrounding
  identifier (see [MiniSurround-surrounding-specification](mini-surround.qmd#minisurround-surrounding-specification)).
...
- General search is done by converting some 2d buffer region (neighborhood
  ...
  (see [MiniSurround-glossary](mini-surround.qmd#minisurround-glossary)). After that, span is converted back into 2d
  ...
  [MiniSurround.config](mini-surround.qmd#minisurround.config)).
...
- For [[count]](https://neovim.io/doc/user/helptag.html?tag=[count]) greater than 1, steps are repeated with current best match
...
#### Parameters {#minisurround.setup-parameters}



<span class="help-syntax-special">{config}</span> `(table|nil)` Module config table. See [MiniSurround.config](mini-surround.qmd#minisurround.config).

#### Usage {#minisurround.setup-usage}
...
#### Defaults {#minisurround.config-defaults}
...
#### Mappings {#minisurround.config-mappings}
...
identifier (supported by [getcharstr()](https://neovim.io/doc/user/helptag.html?tag=getcharstr\(\))) and values - surround specification
(see [MiniSurround-surrounding-specification](mini-surround.qmd#minisurround-surrounding-specification)).
...
- Function returning surround info at <span class="help-syntax-keys">\<input\></span> or <span class="help-syntax-keys">\<output\></span> fields of
  specification is helpful when user input is needed (like asking for
  function name). Use [MiniSurround.user\_input()](mini-surround.qmd#minisurround.user_input) or [MiniInput.get()](mini-input.qmd#miniinput.get).
...
[MiniSurround.gen\_spec](mini-surround.qmd#minisurround.gen_spec)):
...
#### Respect selection type {#minisurround.config-respectselectiontype}
...
#### Search method {#minisurround.config-searchmethod}
...
#### Search suffixes {#minisurround.config-searchsuffixes}


To provide more searching possibilities, [mini.surround](mini-surround.qmd#mini.surround) creates extended
...
- All new mappings behave the same way as if `config.search_method` is set
  to certain search method. They preserve dot-repeat support, respect [[count]](https://neovim.io/doc/user/helptag.html?tag=[count]).
...
#### Setup similar to `tpope/vim-surround` {#minisurround.config-setupsimilartotpopevim-surround}
...
No need to use it directly, everything is setup in [MiniSurround.setup()](mini-surround.qmd#minisurround.setup).

#### Parameters {#minisurround.add-parameters}



<span class="help-syntax-special">{mode}</span> `(string)` Mapping mode (normal by default).
...
This is mainly a wrapper for [input()](https://neovim.io/doc/user/helptag.html?tag=input\(\)) which allows empty string as input,
cancelling with `<Esc>` and `<C-c>`, and slightly modifies prompt. Use it
to ask for input inside function custom surrounding (see [MiniSurround.config](mini-surround.qmd#minisurround.config)).
...
This is a table with two sets of generator functions: <span class="help-syntax-keys">\<input\></span> and <span class="help-syntax-keys">\<output\></span>
(currently empty). Each is a table with function values generating
corresponding surrounding specification.

#### See also {#minisurround.gen_spec-seealso}
...
array of region pairs derived from <span class="help-syntax-keys">\<outer\></span> and <span class="help-syntax-keys">\<inner\></span> captures. It first
...
- Manually create file `after/queries/<language-name>/textobjects.scm` in
  ...
  captures (later used to define surrounding parts). See [lua-treesitter-query](https://neovim.io/doc/user/helptag.html?tag=lua-treesitter-query).
...
#### Parameters {#minisurround.gen_spec.input.treesitter-parameters}



<span class="help-syntax-special">{captures}</span> `(table)` Captures for outer and inner parts of region pair:
  table with <span class="help-syntax-keys">\<outer\></span> and <span class="help-syntax-keys">\<inner\></span> fields with captures for outer
...
#### Return {#minisurround.gen_spec.input.treesitter-return}
...
#### See also {#minisurround.gen_spec.input.treesitter-seealso}



- [MiniSurround-surrounding-specification](mini-surround.qmd#minisurround-surrounding-specification) for how this type of
...
- [MiniAi.gen\_spec.treesitter()](mini-ai.qmd#miniai.gen_spec.treesitter) for similar [mini.ai](mini-ai.qmd#mini.ai) generator.
