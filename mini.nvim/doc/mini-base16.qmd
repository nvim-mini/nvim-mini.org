...
<a name="mini.base16" href="mini-base16.qmd#mini.base16"><b>mini.base16</b></a> Base16 colorscheme creation
...
- Configurable automatic support of cterm colors (see [`highlight-cterm`](https://neovim.io/doc/user/helptag.html?tag=highlight-cterm)).
...
See [`MiniBase16.config`](mini-base16.qmd#minibase16.config) for `config` structure and default values.
...
#### Notes {#minibase16-notes}


1. This is used to create some of plugin's color schemes
   (see [`MiniBase16-color-schemes`](mini-base16.qmd#minibase16-color-schemes)).
2. Using `setup()` doesn't actually create a [`:colorscheme`](https://neovim.io/doc/user/helptag.html?tag=:colorscheme). It basically
   creates a coordinated set of [`highlight-groups`](https://neovim.io/doc/user/helptag.html?tag=highlight-groups). To create your own theme:
    ...
    - Inside "myscheme.lua" call `require('mini.base16').setup()` with your
      palette and only after that set [`g:colors_name`](https://neovim.io/doc/user/helptag.html?tag=g:colors_name) to "myscheme".

---

### Color schemes {#minibase16-color-schemes}

#### Base16 colorschemes {#minibase16-color-schemes-base16colorschemes}


This module comes with several pre-built color schemes. Each of them is
a [`mini.base16`](mini-base16.qmd#mini.base16) theme created with faster version of the following Lua code:

  ```lua
  require('mini.base16').setup({ palette = palette, use_cterm = true })
  ```

Activate them as regular [`:colorscheme`](https://neovim.io/doc/user/helptag.html?tag=:colorscheme) (for example, `:colorscheme minischeme`).
...
By default only 'gui highlighting' (see [`highlight-gui`](https://neovim.io/doc/user/helptag.html?tag=highlight-gui) and
[`'termguicolors'`](https://neovim.io/doc/user/helptag.html?tag='termguicolors')) is supported. To support 'cterm highlighting' (see
[`highlight-cterm`](https://neovim.io/doc/user/helptag.html?tag=highlight-cterm)) supply `config.use_cterm` argument in one of the formats:
...
#### Parameters {#minibase16.setup-parameters}



<span class="help-syntax-special">{config}</span> `(table)` Module config table. See [`MiniBase16.config`](mini-base16.qmd#minibase16.config).

#### Usage {#minibase16.setup-usage}
...
### config {#minibase16.config}

<p align="center">`MiniBase16.config`</p>
#### Defaults {#minibase16.config-defaults}
...
#### Plugin integrations {#minibase16.config-pluginintegrations}
...
- If plugin name (as listed in [`mini.base16`](mini-base16.qmd#mini.base16)) has entry, it is used.
...
### mini_palette() {#minibase16.mini_palette}
...
#### Algorithm design {#minibase16.mini_palette-algorithmdesign}
...
#### Parameters {#minibase16.mini_palette-parameters}



<span class="help-syntax-special">{background}</span> `(string)` Background HEX color (formatted as `#RRGGBB`).

<span class="help-syntax-special">{foreground}</span> `(string)` Foreground HEX color (formatted as `#RRGGBB`).

<span class="help-syntax-special">{accent_chroma}</span> `(number)` Optional positive number (usually between 0
  and 100). Default: chroma of foreground color.

#### Return {#minibase16.mini_palette-return}


`(table)` Table with base16 palette.

#### Usage {#minibase16.mini_palette-usage}
...
### rgb_palette_to_cterm_palette() {#minibase16.rgb_palette_to_cterm_palette}
...
#### Parameters {#minibase16.rgb_palette_to_cterm_palette-parameters}



<span class="help-syntax-special">{palette}</span> `(table)` Table with base16 palette (same as in
  `MiniBase16.config.palette`).

#### Return {#minibase16.rgb_palette_to_cterm_palette-return}


`(table)` Table with base16 palette using [`highlight-cterm`](https://neovim.io/doc/user/helptag.html?tag=highlight-cterm).
