...
NOTE: with default config, built-in LSP mappings [v\_an](https://neovim.io/doc/user/helptag.html?tag=v_an) and [v\_in](https://neovim.io/doc/user/helptag.html?tag=v_in) on Neovim>=0.12
are overridden. Either use different `around_next` / `inside_next` keys or
map manually using [vim.lsp.buf.selection\_range()](https://neovim.io/doc/user/helptag.html?tag=vim.lsp.buf.selection_range\(\)). For example:
