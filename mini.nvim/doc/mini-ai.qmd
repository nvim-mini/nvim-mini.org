...
- Comprehensive builtin textobjects (see more in [`MiniAi-builtin-textobjects`](mini-ai.qmd#miniai-builtin-textobjects)):
    ...
    For more textobjects see [`MiniExtra.gen_ai_spec`](mini-extra.qmd#miniextra.gen_ai_spec).
...
- Set of specification generators to tweak some builtin textobjects (see
  [`MiniAi.gen_spec`](mini-ai.qmd#miniai.gen_spec)).


- Treesitter textobjects (through [`MiniAi.gen_spec.treesitter()`](mini-ai.qmd#miniai.gen_spec.treesitter) helper).
...
#### Comparisons {#miniai-comparisons}
...
- [wellle/targets.vim](https://github.com/wellle/targets.vim):
    ...
    - Doesn't provide any programmatical API for getting information about
      textobjects. 'mini.ai' does it via [`MiniAi.find_textobject()`](mini-ai.qmd#miniai.find_textobject).

    - Has no implementation of "moving to edge of textobject". 'mini.ai'
      does it via [`MiniAi.move_cursor()`](mini-ai.qmd#miniai.move_cursor) and `g[` and `g]` default mappings.
...
- [nvim-treesitter/nvim-treesitter-textobjects](https://github.com/nvim-treesitter/nvim-treesitter-textobjects):
    ...
    - Along with textobject functionality provides a curated and maintained
      ...
      [`MiniAi.gen_spec.treesitter()`](mini-ai.qmd#miniai.gen_spec.treesitter) functionality).
...
### Builtin textobjects {#miniai-builtin-textobjects}
...
- <span class="help-syntax-keys">\<vis_mode\></span> for which Visual mode will be used to select textobject.
  See `opts` argument of [`MiniAi.select_textobject()`](mini-ai.qmd#miniai.select_textobject).
...
### Textobject specification {#miniai-textobject-specification}
...
- Allows callable objects (see [`vim.is_callable()`](https://neovim.io/doc/user/helptag.html?tag=vim.is_callable\(\))) in certain places
  ...
  complexity and computations):
    ...
    - If specification itself is a callable, it will be called with the same
      arguments as [`MiniAi.find_textobject()`](mini-ai.qmd#miniai.find_textobject) and should return one of:
        ...
        - Array of output region(s). Useful for incorporating other
          instruments, like treesitter (see [`MiniAi.gen_spec.treesitter()`](mini-ai.qmd#miniai.gen_spec.treesitter)).
...
See [`MiniAi.gen_spec`](mini-ai.qmd#miniai.gen_spec) for function wrappers to create commonly used
textobject specifications.

---

### Algorithm {#miniai-algorithm}

Search for the textobjects relies on these principles:

- It uses same input data as described in [`MiniAi.find_textobject()`](mini-ai.qmd#miniai.find_textobject),
...
NOTE: with default config, built-in LSP mappings [`v_an`](https://neovim.io/doc/user/helptag.html?tag=v_an) and [`v_in`](https://neovim.io/doc/user/helptag.html?tag=v_in) on Neovim>=0.12
are overridden. Either use different `around_next` / `inside_next` keys or
map manually using [`vim.lsp.buf.selection_range()`](https://neovim.io/doc/user/helptag.html?tag=vim.lsp.buf.selection_range\(\)). For example:
...
### find_textobject() {#miniai.find_textobject}
...
### move_cursor() {#miniai.move_cursor}
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Same as in [`MiniAi.find_textobject()`](mini-ai.qmd#miniai.find_textobject).
  `opts.n_times` means number of actual jumps (important when cursor
  already on the potential jump spot).

---

### gen_spec {#miniai.gen_spec}
...
### gen_spec.argument() {#miniai.gen_spec.argument}
...
### gen_spec.function_call() {#miniai.gen_spec.function_call}
...
### gen_spec.pair() {#miniai.gen_spec.pair}
...
### gen_spec.treesitter() {#miniai.gen_spec.treesitter}
...
- Manually create file 'after/queries/<language name>/textobjects.scm' in
  your [`$XDG_CONFIG_HOME`](https://neovim.io/doc/user/helptag.html?tag=$XDG_CONFIG_HOME) directory. It should contain queries with
...
`(function)` Function with [`MiniAi.find_textobject()`](mini-ai.qmd#miniai.find_textobject) signature which
...
- [`Query:iter_captures()`](https://neovim.io/doc/user/helptag.html?tag=Query:iter_captures\(\)) for how all query captures are iterated in case of
  no 'nvim-treesitter'.

---

### gen_spec.user_prompt() {#miniai.gen_spec.user_prompt}
...
### select_textobject() {#miniai.select_textobject}
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Same as in [`MiniAi.find_textobject()`](mini-ai.qmd#miniai.find_textobject). Extra fields:
