...
<a name="mini.ai" href="mini-ai.qmd#mini.ai"><b>mini.ai</b></a> Extend and create a/i textobjects
...
Enhance some builtin [text-objects](https://neovim.io/doc/user/helptag.html?tag=text-objects) (like [a(](https://neovim.io/doc/user/helptag.html?tag=a\(), [a)](https://neovim.io/doc/user/helptag.html?tag=a\)), [a'](https://neovim.io/doc/user/helptag.html?tag=a'), and more),
...
- Customizable creation of `a`/`i` textobjects using Lua patterns and functions.
  Supports:
    ...
    - Different search methods (see [MiniAi.config](mini-ai.qmd#miniai.config)).
...
- Comprehensive builtin textobjects (see more in [MiniAi-builtin-textobjects](mini-ai.qmd#miniai-builtin-textobjects)):
    ...
    - Default for anything but Latin letters (to fall back to [text-objects](https://neovim.io/doc/user/helptag.html?tag=text-objects)).

    For more textobjects see [MiniExtra.gen\_ai\_spec](mini-extra.qmd#miniextra.gen_ai_spec).
...
- Treesitter textobjects (through [MiniAi.gen\_spec.treesitter()](mini-ai.qmd#miniai.gen_spec.treesitter) helper).
...
information see [MiniAi-textobject-specification](mini-ai.qmd#miniai-textobject-specification) and [MiniAi-algorithm](mini-ai.qmd#miniai-algorithm).
...
See [MiniAi.config](mini-ai.qmd#miniai.config) for available config settings.
...
as `MiniAi.config`. See [mini.nvim-buffer-local-config](mini-nvim.qmd#mini.nvim-buffer-local-config) for more details.
...
- [wellle/targets.vim](https://github.com/wellle/targets.vim):
    ...
    - Has limited support for creating own textobjects: it is constrained
      to pre-defined detection rules. [mini.ai](mini-ai.qmd#mini.ai) allows creating own rules
      via Lua patterns and functions (see [MiniAi-textobject-specification](mini-ai.qmd#miniai-textobject-specification)).

    - Doesn't provide any programmatical API for getting information about
      textobjects. [mini.ai](mini-ai.qmd#mini.ai) does it via [MiniAi.find\_textobject()](mini-ai.qmd#miniai.find_textobject).

    - Has no implementation of "moving to edge of textobject". [mini.ai](mini-ai.qmd#mini.ai)
      does it via [MiniAi.move\_cursor()](mini-ai.qmd#miniai.move_cursor) and `g[` and `g]` default mappings.

    - Both implement the notion of manual "next"/"last" search directions.

    - Implements `A`, `I` operators. [mini.ai](mini-ai.qmd#mini.ai) does not by design: it is
      assumed to be a property of textobject, not operator.

    - Doesn't implement "function call" and "user prompt" textobjects.
      [mini.ai](mini-ai.qmd#mini.ai) does (with `f` and `?` identifiers).
    ...
    - Has limited support for "argument" textobject. Although it works in
      ...
      (like if it is inside quotes or `{}`). [mini.ai](mini-ai.qmd#mini.ai) deals with these cases.
...
- [nvim-treesitter/nvim-treesitter-textobjects](https://github.com/nvim-treesitter/nvim-treesitter-textobjects):
    ...
    - Along with textobject functionality provides a curated and maintained
      ...
      [MiniAi.gen\_spec.treesitter()](mini-ai.qmd#miniai.gen_spec.treesitter) functionality).

    - Both support working with [treesitter-directives](https://neovim.io/doc/user/helptag.html?tag=treesitter-directives) allowing more
      fine-tuned textobjects.

    - Implements only textobjects based on treesitter.

    - Doesn't support [v:count](https://neovim.io/doc/user/helptag.html?tag=v:count).
...
### Builtin textobjects {#miniai-builtin-textobjects .help-syntax-right-anchor}
...
- `2a`/`2i` describe either `2a`/`2i` (support for [v:count](https://neovim.io/doc/user/helptag.html?tag=v:count)) textobjects
...
visual selection, any [operator](https://neovim.io/doc/user/helptag.html?tag=operator) can be used or `g[`/`g]` motions to move
...
- Default textobject is activated for identifiers which are not Latin letters.
  ...
  (see [MiniAi-textobject-specification](mini-ai.qmd#miniai-textobject-specification) and [MiniAi.config](mini-ai.qmd#miniai.config)). Note:
...
### Glossary {#miniai-glossary .help-syntax-right-anchor}

Note: this is similar to [MiniSurround-glossary](mini-surround.qmd#minisurround-glossary).

#### REGION {#miniai-glossary-region}


Table representing region in a buffer. Fields:

- <span class="help-syntax-keys">\<from\></span> and <span class="help-syntax-keys">\<to\></span> for inclusive start and end positions (<span class="help-syntax-keys">\<to\></span> might be
  `nil` to describe empty region). Each position is also a table with
  line <span class="help-syntax-keys">\<line\></span> and column <span class="help-syntax-keys">\<col\></span> (both start at 1).

- <span class="help-syntax-keys">\<vis_mode\></span> for which Visual mode will be used to select textobject.
  See `opts` argument of [MiniAi.select\_textobject()](mini-ai.qmd#miniai.select_textobject).
...
#### PATTERN {#miniai-glossary-pattern}


String describing Lua pattern.

#### SPAN {#miniai-glossary-span}
...
#### NESTED PATTERN {#miniai-glossary-nestedpattern}


Array of patterns aimed to describe nested spans.

#### SPAN MATCHES NESTED PATTERN {#miniai-glossary-spanmatchesnestedpattern}
...
#### COMPOSED PATTERN {#miniai-glossary-composedpattern}
...
2. Either "balanced `()` with inner edge space" or "balanced `[]` with no
   inner edge space", both with 5 or more characters:
...
#### SPAN MATCHES COMPOSED PATTERN {#miniai-glossary-spanmatchescomposedpattern}
...
### Textobject specification {#miniai-textobject-specification .help-syntax-right-anchor}

Textobject specification has a structure of composed pattern (see
[MiniAi-glossary](mini-ai.qmd#miniai-glossary)) with two differences:
...
- Allows callable objects (see [vim.is\_callable()](https://neovim.io/doc/user/helptag.html?tag=vim.is_callable\(\))) in certain places
  ...
  complexity and computations):
    ...
    - If specification itself is a callable, it will be called with the same
      arguments as [MiniAi.find\_textobject()](mini-ai.qmd#miniai.find_textobject) and should return one of:
        ...
        - Array of output region(s). Useful for incorporating other
          instruments, like treesitter (see [MiniAi.gen\_spec.treesitter()](mini-ai.qmd#miniai.gen_spec.treesitter)).
...
See [MiniAi.gen\_spec](mini-ai.qmd#miniai.gen_spec) for function wrappers to create commonly used
textobject specifications.

---

### Algorithm {#miniai-algorithm .help-syntax-right-anchor}

Search for the textobjects relies on these principles:

- It uses same input data as described in [MiniAi.find\_textobject()](mini-ai.qmd#miniai.find_textobject),
...
- Textobject specification is constructed based on textobject identifier
  (see [MiniAi-textobject-specification](mini-ai.qmd#miniai-textobject-specification)).
...
- General search is done by converting some 2d buffer region (neighborhood
  ...
  inside string (see [MiniAi-glossary](mini-ai.qmd#miniai-glossary)). After that, span is converted back
  ...
  (see [MiniAi.config](mini-ai.qmd#miniai.config)).
...
- For the sake of increasing functionality, during iteration over all
  ...
  specially.
    ...
    - `x.-y` (`x` and `y` are different strings). It results only in matches with
      ...
      frontier pattern `%f[]` (see examples in [MiniAi-textobject-specification](mini-ai.qmd#miniai-textobject-specification)).
...
#### Parameters {#miniai.setup-parameters}



<span class="help-syntax-special">{config}</span> `(table|nil)` Module config table. See [MiniAi.config](mini-ai.qmd#miniai.config).

#### Usage {#miniai.setup-usage}
...
#### Defaults {#miniai.config-defaults}
...
#### Custom textobjects {#miniai.config-customtextobjects}
...
(supported by [getcharstr()](https://neovim.io/doc/user/helptag.html?tag=getcharstr\(\)), except <span class="help-syntax-keys">\<Esc\></span> and <span class="help-syntax-keys">\<C-c\></span> which are used to cancel)
and values - textobject specification (see [MiniAi-textobject-specification](mini-ai.qmd#miniai-textobject-specification)).

General recommendations:

- This can be used to override builtin ones ([MiniAi-builtin-textobjects](mini-ai.qmd#miniai-builtin-textobjects)).
...
- Keys should use character representation which can be [getcharstr()](https://neovim.io/doc/user/helptag.html?tag=getcharstr\(\)) output.
...
There are more example specifications in [MiniAi-textobject-specification](mini-ai.qmd#miniai-textobject-specification).

#### Search method {#miniai.config-searchmethod}
...
#### Mappings {#miniai.config-mappings}
...
##### Overriding default `an` and `in` {#miniai.config-mappings-overridingdefaultanandin}


<a name="miniai-default-an-in" href="mini-ai.qmd#miniai-default-an-in"><b>MiniAi-default-an-in</b></a>

Default `around_next` / `inside_next` mappings override newly added in Neovim=0.12
built-in mappings [v\_an](https://neovim.io/doc/user/helptag.html?tag=v_an) and [v\_in](https://neovim.io/doc/user/helptag.html?tag=v_in). This is intentional for better usability
and backwards compatibility. There are several ways to work around this:


- Remap Neovim's `an` / `in` to something else before calling [MiniAi.setup()](mini-ai.qmd#miniai.setup):
...
- Use [MiniAi.gen\_spec.treesitter()](mini-ai.qmd#miniai.gen_spec.treesitter) with one or many captures for versatile
...
- Use other values for "next" / "last" variants. For example:
...
##### Overriding default `al` and `il` {#miniai.config-mappings-overridingdefaultalandil}


<a name="miniai-default-al-il" href="mini-ai.qmd#miniai-default-al-il"><b>MiniAi-default-al-il</b></a>

Default `around_last` / `inside_last` mappings override newly added in Neovim=0.13
built-in mappings [al](https://neovim.io/doc/user/helptag.html?tag=al) and [il](https://neovim.io/doc/user/helptag.html?tag=il). This is intentional for better usability
and backwards compatibility. There are several ways to work around this:


- Use [MiniExtra.gen\_ai\_spec.buffer()](mini-extra.qmd#miniextra.gen_ai_spec.buffer) and [MiniExtra.gen\_ai\_spec.line()](mini-extra.qmd#miniextra.gen_ai_spec.line) to
  create custom textobjects for buffer and line:
...
- Use other values for "next" / "last" variants. See [MiniAi-default-an-in](mini-ai.qmd#miniai-default-an-in).
...
#### Parameters {#miniai.find_textobject-parameters}



<span class="help-syntax-special">{ai_type}</span> `(string)` One of `'a'` or `'i'`.

<span class="help-syntax-special">{id}</span> `(string)` Single character string representing textobject id. It is
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Options. Possible fields:
  ...
  - <span class="help-syntax-keys">\<n_lines\></span> - Number of lines within which textobject is searched.
    Default: `config.n_lines` (see [MiniAi.config](mini-ai.qmd#miniai.config)).

  - <span class="help-syntax-keys">\<n_times\></span> - Number of times to perform a consecutive search. Each one
    is done with reference region being previous found textobject region.
    Default: 1.

  - <span class="help-syntax-keys">\<reference_region\></span> - region to try to cover (see [MiniAi-glossary](mini-ai.qmd#miniai-glossary)). It
    is guaranteed that output region will not be inside or equal to this one.
    Default: empty region at cursor position.

  - <span class="help-syntax-keys">\<search_method\></span> - Search method. Default: `config.search_method`.

#### Return {#miniai.find_textobject-return}
...
#### Parameters {#miniai.move_cursor-parameters}



<span class="help-syntax-special">{side}</span> `(string)` One of `'left'` or `'right'`.

<span class="help-syntax-special">{ai_type}</span> `(string)` One of `'a'` or `'i'`.

<span class="help-syntax-special">{id}</span> `(string)` Single character string representing textobject id.

<span class="help-syntax-special">{opts}</span> `(table|nil)` Same as in [MiniAi.find\_textobject()](mini-ai.qmd#miniai.find_textobject).
...
#### Parameters {#miniai.gen_spec.argument-parameters}



<span class="help-syntax-special">{opts}</span> `(table|nil)` Options. Allowed fields:

  - <span class="help-syntax-keys">\<brackets\></span> - array of patterns for outer balanced brackets.
    Default: `{ '%b()', '%b[]', '%b{}' }` (any `()`, `[]`, or `{}` can
    enclose arguments).

  - <span class="help-syntax-keys">\<separator\></span> - separator pattern. Default: `','`.
  ...
  - <span class="help-syntax-keys">\<exclude_regions\></span> - array with patterns for regions inside which
...
#### Parameters {#miniai.gen_spec.function_call-parameters}



<span class="help-syntax-special">{opts}</span> `(table|nil)` Options. Allowed fields:

  - <span class="help-syntax-keys">\<name_pattern\></span> - string pattern of character set allowed in function name.
...
#### Parameters {#miniai.gen_spec.pair-parameters}



<span class="help-syntax-special">{left}</span> `(string)` Left edge.

<span class="help-syntax-special">{right}</span> `(string)` Right edge.

<span class="help-syntax-special">{opts}</span> `(table|nil)` Options. Possible fields:

  - <span class="help-syntax-keys">\<type\></span> - Type of a pair. One of `'non-balanced'` (default), `'balanced'`,
  `'greedy'`.
...
query (see [vim.treesitter.query.get()](https://neovim.io/doc/user/helptag.html?tag=vim.treesitter.query.get\(\))).
...
- Manually create file `after/queries/<language-name>/textobjects.scm` in
  your [\$XDG\_CONFIG\_HOME](https://neovim.io/doc/user/helptag.html?tag=$XDG_CONFIG_HOME) directory. It should contain queries with
  captures (later used to define textobjects). See [lua-treesitter-query](https://neovim.io/doc/user/helptag.html?tag=lua-treesitter-query).
...
- Be sure that query files don't contain unknown [treesitter-directives](https://neovim.io/doc/user/helptag.html?tag=treesitter-directives)
  (like `#make-range!`, for example). Otherwise textobject for such capture
  might not be found as [lua-treesitter-core](https://neovim.io/doc/user/helptag.html?tag=lua-treesitter-core) won't treat them as captures.
  Verify with `:=vim.treesitter.query.get('lang', 'textobjects')` and see
  if the target capture is recognized as one.

- It uses buffer's [filetype](https://neovim.io/doc/user/helptag.html?tag=filetype) to determine query language.
...
#### Parameters {#miniai.gen_spec.treesitter-parameters}



<span class="help-syntax-special">{ai_captures}</span> `(table)` Captures for `a` and `i` textobjects: table with
  <span class="help-syntax-keys">\<a\></span> and <span class="help-syntax-keys">\<i\></span> fields with captures for `a` and `i` textobjects respectively.
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Options. Possible values:
  ...
  - <span class="help-syntax-keys">\<use_nvim_treesitter\></span> - whether to try to use `nvim-treesitter` plugin
    ...
    used. However, as [lua-treesitter-core](https://neovim.io/doc/user/helptag.html?tag=lua-treesitter-core) methods are more capable now,
    the option will soon be removed. Only present for backward compatibility.
    Default: `false`.

#### Return {#miniai.gen_spec.treesitter-return}


`(function)` Function with [MiniAi.find\_textobject()](mini-ai.qmd#miniai.find_textobject) signature which
  returns array of current buffer regions representing matches for
  corresponding (`a` or `i`) treesitter capture.

#### See also {#miniai.gen_spec.treesitter-seealso}



- [MiniAi-textobject-specification](mini-ai.qmd#miniai-textobject-specification) for how this type of textobject
  specification is processed.

- [vim.treesitter.query.get()](https://neovim.io/doc/user/helptag.html?tag=vim.treesitter.query.get\(\)) for how query is fetched.

- [Query:iter\_captures()](https://neovim.io/doc/user/helptag.html?tag=Query:iter_captures\(\)) for how all query captures are iterated in case of
  no `nvim-treesitter`.
...
- Ask user for left and right textobject edges as raw strings (no pattern).
  It uses [MiniInput.get()](mini-input.qmd#miniinput.get) (if enabled) or |input().
...
Used for [MiniAi-builtin-textobjects](mini-ai.qmd#miniai-builtin-textobjects) with identifier `?`.

#### Return {#miniai.gen_spec.user_prompt-return}
...
#### Parameters {#miniai.select_textobject-parameters}
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Same as in [MiniAi.find\_textobject()](mini-ai.qmd#miniai.find_textobject). Extra fields:

  - <span class="help-syntax-keys">\<vis_mode\></span> - One of `'v'`, `'V'`, or `'\22'` (escaped version of `'<C-v>'`).
    Default: Latest visual mode.

  - <span class="help-syntax-keys">\<operator_pending\></span> - Whether selection is for Operator-pending mode.
