...
- [Neovide](https://neovide.dev/):
    ...
    - Neovide is a standalone GUI which has more control over its animations.
      While [mini.animate](mini-animate.qmd#mini.animate) works inside terminal emulator (with all its
      limitations, like lack of pixel-size control over animations).

    - Neovide animates cursor movement across screen, while [mini.animate](mini-animate.qmd#mini.animate) -
      as it moves across same buffer.

    - Neovide has fixed number of animation effects per action, while
      [mini.animate](mini-animate.qmd#mini.animate) is fully customizable.

    - [mini.animate](mini-animate.qmd#mini.animate) implements animations for window open/close, while
      Neovide does not.

- [edluffy/specs.nvim](https://github.com/edluffy/specs.nvim):

    - [mini.animate](mini-animate.qmd#mini.animate) approaches cursor movement visualization via
...
- [karb94/neoscroll.nvim](https://github.com/karb94/neoscroll.nvim):
    ...
    - Scroll animation is triggered only inside dedicated mappings.
      [mini.animate](mini-animate.qmd#mini.animate) animates scroll resulting from any window view change.
...
- [anuvyklack/windows.nvim](https://github.com/anuvyklack/windows.nvim):
    ...
    - Resize animation is done only within custom commands and mappings,
      while [mini.animate](mini-animate.qmd#mini.animate) animates any resize with appropriate values of
      ['winheight'](https://neovim.io/doc/user/helptag.html?tag='winheight') / ['winwidth'](https://neovim.io/doc/user/helptag.html?tag='winwidth') and ['winminheight'](https://neovim.io/doc/user/helptag.html?tag='winminheight') / ['winminwidth'](https://neovim.io/doc/user/helptag.html?tag='winminwidth')).

#### Highlight groups {#module-highlightgroups}


<a name="minianimate-hl-groups" href="mini-animate.qmd#minianimate-hl-groups"><b>MiniAnimate-hl-groups</b></a>
...
- <a name="minianimate-timing" href="mini-animate.qmd#minianimate-timing"><b>MiniAnimate-timing</b></a> Every animation is a non-blockingly scheduled series of
  ...
  See [MiniAnimate.gen\_timing](mini-animate.qmd#minianimate.gen_timing) for builtin timing functions.
...
- <a name="minianimate-done-event" href="mini-animate.qmd#minianimate-done-event"><b>MiniAnimate-done-event</b></a> Every animation triggers custom [User](https://neovim.io/doc/user/helptag.html?tag=User) event when it
  ...
  you can use [MiniAnimate.execute\_after()](mini-animate.qmd#minianimate.execute_after) (usually preferred in mappings).
...
See [MiniAnimate.gen\_path](mini-animate.qmd#minianimate.gen_path) for builtin path generators.
...
See [MiniAnimate.gen\_subscroll](mini-animate.qmd#minianimate.gen_subscroll) for builtin subscroll generators.
...
- As scroll animation is essentially a precisely scheduled non-blocking
  subscrolls, this has two important interconnected consequences:
    ...
    - If another scroll is attempted during the animation, it is done based
      on the CURRENTLY VISIBLE window view. Example: if user presses [CTRL-D](https://neovim.io/doc/user/helptag.html?tag=CTRL-D)
      and then [CTRL-U](https://neovim.io/doc/user/helptag.html?tag=CTRL-U) when animation is half done, window will not display
      the previous view half of ['scroll'](https://neovim.io/doc/user/helptag.html?tag='scroll') above it. This especially affects
      mouse wheel scrolling, as each its turn results in a new scroll for
      number of lines defined by ['mousescroll'](https://neovim.io/doc/user/helptag.html?tag='mousescroll'). Tweak it to your liking.

    - It breaks the use of several relative scrolling commands in the same
      command. Use [MiniAnimate.execute\_after()](mini-animate.qmd#minianimate.execute_after) to schedule action after
...
- Default timing might conflict with scrolling via holding a key (like `j` or `k`
  with ['wrap'](https://neovim.io/doc/user/helptag.html?tag='wrap') enabled) due to high key repeat rate: next scroll is done before
...
after something like [CTRL-W\_K](https://neovim.io/doc/user/helptag.html?tag=CTRL-W_K). Its visualization step consists from setting
...
See [MiniAnimate.gen\_subresize](mini-animate.qmd#minianimate.gen_subresize) for builtin subresize generators.
...
argument of [nvim\_open\_win()](https://neovim.io/doc/user/helptag.html?tag=nvim_open_win\(\))). Its length determines number of animation steps.
Example:
...
window's ['winblend'](https://neovim.io/doc/user/helptag.html?tag='winblend') option. Note, that it is called for current step (so
...
See [MiniAnimate.gen\_winconfig](mini-animate.qmd#minianimate.gen_winconfig) for builtin window config generators.
See [MiniAnimate.gen\_winblend](mini-animate.qmd#minianimate.gen_winblend) for builtin window transparency generators.
...
[MiniAnimate.is\_active()](mini-animate.qmd#minianimate.is_active)). Else, schedule its execution until after
...
<span class="help-syntax-special">{animation_type}</span> `(string)` One of supported animation types
  (as in [MiniAnimate.is\_active()](mini-animate.qmd#minianimate.is_active)).
...
- Asynchronous execution is done with [uv.new\_timer()](https://neovim.io/doc/user/helptag.html?tag=uv.new_timer\(\)). It only allows
...
[MiniIndentscope.gen\_animation](mini-indentscope.qmd#miniindentscope.gen_animation) for similar concept in [mini.indentscope](mini-indentscope.qmd#mini.indentscope).
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Options that control generator. Possible keys:
  ...
  - <span class="help-syntax-keys">\<from\></span> `(number)` - initial value of ['winblend'](https://neovim.io/doc/user/helptag.html?tag='winblend').

  - <span class="help-syntax-keys">\<to\></span> `(number)` - final value of ['winblend'](https://neovim.io/doc/user/helptag.html?tag='winblend').
