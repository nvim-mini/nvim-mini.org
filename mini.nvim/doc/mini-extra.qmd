...
Extra useful functionality which is not essential enough for other [mini.nvim](mini-nvim.qmd#mini.nvim)
...
- This module requires only those [mini.nvim](mini-nvim.qmd#mini.nvim) modules which are needed for
  a particular functionality: [mini.pick](mini-pick.qmd#mini.pick) for pickers, etc.
...
- [nvim-telescope/telescope.nvim](https://github.com/nvim-telescope/telescope.nvim):
    ...
    - With [MiniExtra.pickers](mini-extra.qmd#miniextra.pickers), [mini.pick](mini-pick.qmd#mini.pick) is reasonably on par when it comes
      to built-in pickers.
...
- Differences with [MiniIndentscope.textobject()](mini-indentscope.qmd#miniindentscope.textobject):
    ...
    - This textobject always treats blank lines on top and bottom of `i`
      textobject as part of it, while [mini.indentscope](mini-indentscope.qmd#mini.indentscope) can configure that.

    - This textobject can select non-covering scopes, while [mini.indentscope](mini-indentscope.qmd#mini.indentscope)
      can not (by design).

    - In this textobject scope computation is done only by "casting rays" from
      top to bottom and not in both ways as in [mini.indentscope](mini-indentscope.qmd#mini.indentscope).
...
A table with [mini.pick](mini-pick.qmd#mini.pick) pickers (which is a hard dependency).
Notes:
...
Pick from Neovim built-in ([Ex-command](https://neovim.io/doc/user/helptag.html?tag=Ex-command)) and [user-commands](https://neovim.io/doc/user/helptag.html?tag=user-commands).
Notes:
...
Matches from [mini.hipatterns](mini-hipatterns.qmd#mini.hipatterns) picker
...
- Requires [mini.hipatterns](mini-hipatterns.qmd#mini.hipatterns).
...
<span class="help-syntax-special">{local_opts}</span> `(table|nil)` Options defining behavior of this particular picker.
  ...
  - <span class="help-syntax-keys">\<scope\></span> `(string)` - one of "all" (buffers with enabled [mini.hipatterns](mini-hipatterns.qmd#mini.hipatterns))
...
Pick manual page (like described in [ft-man-plugin](https://neovim.io/doc/user/helptag.html?tag=ft-man-plugin)).
Notes:

- Depends on [:Man](https://neovim.io/doc/user/helptag.html?tag=:Man) command to preview and choose items.

- Shows page in the target window. Use [MiniPick-actions-choose](mini-pick.qmd#minipick-actions-choose) to split.

#### Parameters {#miniextra.pickers.manpages-parameters}
...
#### Return {#miniextra.pickers.manpages-return}
...
Visit paths from [mini.visits](mini-visits.qmd#mini.visits) picker
...
- Requires [mini.visits](mini-visits.qmd#mini.visits).
...
Visit labels from [mini.visits](mini-visits.qmd#mini.visits) picker
