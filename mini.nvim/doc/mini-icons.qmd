...
<a name="mini.icons" href="mini-icons.qmd#mini.icons"><b>mini.icons</b></a> Icon provider
...
- Provide icons with their highlighting via a single [MiniIcons.get()](mini-icons.qmd#miniicons.get) for
...
- Integration with [vim.filetype.add()](https://neovim.io/doc/user/helptag.html?tag=vim.filetype.add\(\)) and [vim.filetype.match()](https://neovim.io/doc/user/helptag.html?tag=vim.filetype.match\(\)).


- Mocking methods of `nvim-tree/nvim-web-devicons` for better integrations
  with plugins outside [mini.nvim](mini-nvim.qmd#mini.nvim). See [MiniIcons.mock\_nvim\_web\_devicons()](mini-icons.qmd#miniicons.mock_nvim_web_devicons).


- Tweaking built-in maps for "LSP kind" to include icons. In particular, this
  makes [mini.completion](mini-completion.qmd#mini.completion) use icons in LSP step. See [MiniIcons.tweak\_lsp\_kind()](mini-icons.qmd#miniicons.tweak_lsp_kind).
...
- It is not a goal to become a collection of icons for as much use cases as
  ...
  built-in in each category (see [MiniIcons.get()](mini-icons.qmd#miniicons.get)).
  The main supported category is "filetype".

Sources with more details:

- [MiniIcons-in-other-plugins](mini-icons.qmd#miniicons-in-other-plugins) (for plugin authors)
...
See [MiniIcons.config](mini-icons.qmd#miniicons.config) for `config` structure and default values.
...
- [nvim-tree/nvim-web-devicons](https://github.com/nvim-tree/nvim-web-devicons) (for users):
    ...
    - Using this module can occasionally result in small delays when used
      ...
      in [mini.files](mini-files.qmd#mini.files)). This is due to using [vim.filetype.match()](https://neovim.io/doc/user/helptag.html?tag=vim.filetype.match\(\)) fallback and
    ...
    - This module provides [MiniIcons.mock\_nvim\_web\_devicons()](mini-icons.qmd#miniicons.mock_nvim_web_devicons) function which
      when called imitates installed `nvim-web-devicons` plugin to support
      other plugins which do not provide [mini.icons](mini-icons.qmd#mini.icons) yet.
...
- [nvim-tree/nvim-web-devicons](https://github.com/nvim-tree/nvim-web-devicons) (for plugin developers):
    ...
    - Both have main "get icon" type of function:
        ...
        - This module always returns icon data possibly falling back to
          ...
          a fallback (see [MiniIcons.get()](mini-icons.qmd#miniicons.get)).


        - This module uses [vim.filetype.match()](https://neovim.io/doc/user/helptag.html?tag=vim.filetype.match\(\)) as a fallback for "file"
    ...
    - Different sets of supported categories (see [MiniIcons.config](mini-icons.qmd#miniicons.config)):
...
<a name="miniicons-hl-groups" href="mini-icons.qmd#miniicons-hl-groups"><b>MiniIcons-hl-groups</b></a>
...
<a name="miniicons-in-other-plugins" href="mini-icons.qmd#miniicons-in-other-plugins"><b>MiniIcons-in-other-plugins</b></a>
...
- Use [MiniIcons.get()](mini-icons.qmd#miniicons.get) function to get icon string and more data about it.
...
- For file icons prefer using full path instead of relative or only basename.
  ...
  The [MiniIcons.config](mini-icons.qmd#miniicons.config) has an example of that.
...
#### Parameters {#miniicons.setup-parameters}



<span class="help-syntax-special">{config}</span> `(table|nil)` Module config table. See [MiniIcons.config](mini-icons.qmd#miniicons.config).

#### Usage {#miniicons.setup-usage}
...
#### Defaults {#miniicons.config-defaults}
...
#### Customization per category {#miniicons.config-customizationpercategory}
...
Customization should be done by supplying a table with <span class="help-syntax-keys">\<glyph\></span> (icon glyph)
and/or <span class="help-syntax-keys">\<hl\></span> (name of highlight group) string fields as a value for an icon
name entry. Example:
...
- These customizations only take effect inside [MiniIcons.setup()](mini-icons.qmd#miniicons.setup) call.
...
- Use lower case names for categories which are matched ignoring case.
  See [MiniIcons.get()](mini-icons.qmd#miniicons.get) for more details.

#### Using extension during file resolution {#miniicons.config-usingextensionduringfileresolution}
...
resolution (see [MiniIcons.get()](mini-icons.qmd#miniicons.get) for more details).
...
ignored in order for resolution to reach [vim.filetype.match()](https://neovim.io/doc/user/helptag.html?tag=vim.filetype.match\(\)) stage. This
...
- Output is cached after the first call to increase performance of next calls
  with same arguments. To reset cache, call [MiniIcons.setup()](mini-icons.qmd#miniicons.setup).


- To increase first call performance for "extension" and "file" categories,
  add frequently used values in [MiniIcons.config](mini-icons.qmd#miniicons.config). They will be preferred
  over executing [vim.filetype.match()](https://neovim.io/doc/user/helptag.html?tag=vim.filetype.match\(\)).


- Matching icon name for "file" and "directory" categories is done exactly
  and respecting case. Others are done ignoring case.

#### Parameters {#miniicons.get-parameters}



<span class="help-syntax-special">{category}</span> `(string)` Category name. Supported categories:

  - `'default'` - icon data used as fallback for any category.
    Icon names:

      - <span class="help-syntax-keys">\<Input\></span>: any supported category name.

      - <span class="help-syntax-keys">\<Built-in\></span>: only supported category names.
  ...
  - `'directory'` - icon data for directory path.
    Icon names:
      ...
      - <span class="help-syntax-keys">\<Input\></span>: any string, but only basename is used. Works with not present
        paths (no check is done).

      - <span class="help-syntax-keys">\<Built-in\></span>: popular directory names not tied to language/software
  ...
  - `'extension'` - icon data for extension.
    Icon names:
      ...
      - <span class="help-syntax-keys">\<Input\></span>: any string (without extra dot prefix).

      - <span class="help-syntax-keys">\<Built-in\></span>: popular extensions without associated filetype plus a set
    ...
    Icon data is attempted to be resolved in the following order:
      ...
      - Filetype as a result of [vim.filetype.match()](https://neovim.io/doc/user/helptag.html?tag=vim.filetype.match\(\)) with placeholder
  ...
  - `'file'` - icon data for file path.
    Icon names:
      ...
      - <span class="help-syntax-keys">\<Input\></span>: any string. Works with not present paths (no check is done).

      - <span class="help-syntax-keys">\<Built-in\></span>: popular file names not tied to language/software
    ...
    Icon data is attempted to be resolved in the following order:
      ...
      - Filetype as a result of [vim.filetype.match()](https://neovim.io/doc/user/helptag.html?tag=vim.filetype.match\(\)) with full input (not
  ...
  - `'filetype'` - icon data for ['filetype'](https://neovim.io/doc/user/helptag.html?tag='filetype') values.
    Icon names:

      - <span class="help-syntax-keys">\<Input\></span>: any string.

      - <span class="help-syntax-keys">\<Built-in\></span>: any filetype that is reasonably used in Neovim ecosystem.
  ...
  - `'lsp'` - icon data for various "LSP kind" values.
    Icon names:
      ...
      - <span class="help-syntax-keys">\<Built-in\></span>: only namespace entries from LSP specification that can
  ...
  - `'os'` - icon data for popular operating systems.
    Icon names:
      ...
      - <span class="help-syntax-keys">\<Built-in\></span>: only operating systems which have `nf-md-*` class icon.
...
<span class="help-syntax-special">{name}</span> `(string)` Icon name within category. Use [MiniIcons.list()](mini-icons.qmd#miniicons.list) to get icon
  names which are explicitly supported for specific category.

#### Return {#miniicons.get-return}
...
`(...)` Tuple of icon string, highlight group name it is suggested to be
  ...
  as a result of fallback to default. Example:
...
#### Parameters {#miniicons.list-parameters}



<span class="help-syntax-special">{category}</span> `(string)` Category name supported by [MiniIcons.get()](mini-icons.qmd#miniicons.get).

#### Return {#miniicons.list-return}
...
`(table)` Array of icon names which are explicitly supported for category.
  ...
  via their fallback to using [vim.filetype.match()](https://neovim.io/doc/user/helptag.html?tag=vim.filetype.match\(\)) with `'filetype'` category.
...
using [MiniIcons.get()](mini-icons.qmd#miniicons.get) equivalent.

This function is useful if any plugins relevant to you depend solely on
`nvim-web-devicons` and have not yet added an integration with [mini.icons](mini-icons.qmd#mini.icons).
...
Update in place appropriate maps in [vim.lsp.protocol](https://neovim.io/doc/user/helptag.html?tag=vim.lsp.protocol) (`CompletionItemKind`
...
- Makes [mini.completion](mini-completion.qmd#mini.completion) show icons, as it uses built-in protocol map.

- Results in loading whole `vim.lsp` module, so might add significant amount
  of time on startup. Call it lazily. For example, with [MiniDeps.later()](mini-deps.qmd#minideps.later):
...
#### Parameters {#miniicons.tweak_lsp_kind-parameters}



<span class="help-syntax-special">{mode}</span> `(string|nil)` One of "prepend" (default), "append", "replace".
