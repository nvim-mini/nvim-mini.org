...
- Mocking methods of 'nvim-tree/nvim-web-devicons' for better integrations
  with plugins outside 'mini.nvim'. See [MiniIcons.mock\_nvim\_web\_devicons()](mini-icons.qmd#miniicons.mock_nvim_web_devicons).
...
<span class="help-syntax-special">{category}</span> `(string)` Category name. Supported categories:
  ...
  - `'lsp'` - icon data for various "LSP kind" values.
    Icon names:
      ...
      - <span class="help-syntax-keys">\<Built-in\></span>: only namesspace entries from LSP specification that are
