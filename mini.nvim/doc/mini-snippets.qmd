...
- [abeldekat/cmp-mini-snippets](https://github.com/abeldekat/cmp-mini-snippets):
    ...
    - A source for [hrsh7th/nvim-cmp](https://github.com/hrsh7th/nvim-cmp)
      that integrates [mini.snippets](mini-snippets.qmd#mini.snippets).

#### Highlight groups {#module-highlightgroups}


<a name="minisnippets-hl-groups" href="mini-snippets.qmd#minisnippets-hl-groups"><b>MiniSnippets-hl-groups</b></a>
...
an LSP server that matches and provides snippets loaded with [mini.snippets](mini-snippets.qmd#mini.snippets).
To match with completion engine, use `start_lsp_server({ match = false })`.

#### Management {#minisnippets-overview-management}


Out of the box [mini.snippets](mini-snippets.qmd#mini.snippets) doesn't load any snippets, it should be done
...
- Put files in "snippets" subdirectory of any path in ['runtimepath'](https://neovim.io/doc/user/helptag.html?tag='runtimepath') (like
...
- Set up [mini.snippets](mini-snippets.qmd#mini.snippets) as recommended in [MiniSnippets-examples](mini-snippets.qmd#minisnippets-examples).
...
Output loads files from `snippets/` subdirectories of ['runtimepath'](https://neovim.io/doc/user/helptag.html?tag='runtimepath') matching
...
['runtimepath'](https://neovim.io/doc/user/helptag.html?tag='runtimepath'). This is useful to simultaneously read several similarly
named files from different sources. Order from ['runtimepath'](https://neovim.io/doc/user/helptag.html?tag='runtimepath') is preserved.

Typical case is loading snippets for a language from files like `xxx.{json,lua}`
but located in different `snippets/` directories inside ['runtimepath'](https://neovim.io/doc/user/helptag.html?tag='runtimepath').
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Options. Possible fields:
  ...
  - <span class="help-syntax-keys">\<cache\></span> `(boolean)` - whether to use cached output. Default: `true`.
    ...
    ['runtimepath'](https://neovim.io/doc/user/helptag.html?tag='runtimepath') value and snippet files do not change during Neovim session.
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Options. Possible fields:
  ...
  - <span class="help-syntax-keys">\<context\></span> `(any)` - Context used as an argument for callable snippet data.
    ...
    (allows different snippets in injected languages), ['filetype'](https://neovim.io/doc/user/helptag.html?tag='filetype') otherwise.
...
- Insert snippet at cursor:
    ...
    - Add snippet's text. Lines are split at "\n".
      Indent and left comment leaders (inferred from ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring') and
      ['comments'](https://neovim.io/doc/user/helptag.html?tag='comments')) of current line are repeated on the next.
      Tabs ("\t") are expanded according to ['expandtab'](https://neovim.io/doc/user/helptag.html?tag='expandtab') and ['shiftwidth'](https://neovim.io/doc/user/helptag.html?tag='shiftwidth').
...
- If tabstop has choices, all of them are shown after each jump and deleting
  ...
  Works best when ['completeopt'](https://neovim.io/doc/user/helptag.html?tag='completeopt') option contains `menuone` and `noselect` flags.
