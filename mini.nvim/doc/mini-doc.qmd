...
- Some settings tips that might make writing annotation comments easier:
    ...
    - Set up appropriate 'formatoptions' (see also [`fo-table`](https://neovim.io/doc/user/helptag.html?tag=fo-table)). Consider
...
#### Comparisons {#minidoc-comparisons}
...
### Data structures {#minidoc-data-structures}
...
- Hooks are expected to be functions. Their default values might do many
  ...
  [`MiniDoc.default_hooks`](mini-doc.qmd#minidoc.default_hooks).

---

### current {#minidoc.current}
...
### default_hooks {#minidoc.default_hooks}
...
- Hooks for sections describing some "variable-like" object ('@class',
  '@field', '@param') automatically enclose first word in '<span class="help-syntax-special">{}</span>'.
...
<span class="help-syntax-keys">\<MiniDoc.config.script_path\></span>` with current Lua runtime while caching and
...
### afterlines_to_code() {#minidoc.afterlines_to_code}
