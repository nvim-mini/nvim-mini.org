...
<a name="mini.doc" href="mini-doc.qmd#mini.doc"><b>mini.doc</b></a> Generate Neovim help files
...
- Keep documentation next to code by writing EmmyLua-like annotation
  ...
  guide in [help-writing](https://neovim.io/doc/user/helptag.html?tag=help-writing). However, custom hooks are allowed at many
...
- Generation is done by processing a set of ordered files line by line.
  ...
  "afterline" of documentation block). See [MiniDoc.generate()](mini-doc.qmd#minidoc.generate) for more
  details.


- Processing is done by using nested data structures (section, block, file,
  doc) describing certain parts of help file. See [MiniDoc-data-structures](mini-doc.qmd#minidoc-data-structures)
  for more details.


- Project specific script can be written as plain Lua file with
  configuratble path. See [MiniDoc.generate()](mini-doc.qmd#minidoc.generate) for more details.
...
See [MiniDoc.config](mini-doc.qmd#minidoc.config) for available config settings.
...
- Some settings tips that might make writing annotation comments easier:
    ...
    - Set up appropriate ['comments'](https://neovim.io/doc/user/helptag.html?tag='comments') for `lua` file type to respect
      EmmyLua-like's `---` comment leader. Value `:---,:--` seems to work.

    - Set up appropriate ['formatoptions'](https://neovim.io/doc/user/helptag.html?tag='formatoptions') (see also [fo-table](https://neovim.io/doc/user/helptag.html?tag=fo-table)). Consider
      adding `j`, `n`, `q`, and `r` flags.

    - Set up appropriate ['formatlistpat'](https://neovim.io/doc/user/helptag.html?tag='formatlistpat') to help auto-formatting lists (if
      `n` flag is added to ['formatoptions'](https://neovim.io/doc/user/helptag.html?tag='formatoptions')). One suggestion (not entirely
...
### Data structures {#minidoc-data-structures .help-syntax-right-anchor}
...
- `Doc`:
    ...
    - `input` - array of input file paths (as in [MiniDoc.generate()](mini-doc.qmd#minidoc.generate)).

    - `output` - output path (as in [MiniDoc.generate()](mini-doc.qmd#minidoc.generate)).

    - `config` - configuration used (as in [MiniDoc.generate()](mini-doc.qmd#minidoc.generate)).
...
#### Parameters {#minidoc.setup-parameters}



<span class="help-syntax-special">{config}</span> `(table|nil)` Module config table. See [MiniDoc.config](mini-doc.qmd#minidoc.config).

#### Usage {#minidoc.setup-usage}
...
#### Defaults {#minidoc.config-defaults}
...
- Hooks are expected to be functions. Their default values might do many
  ...
  [MiniDoc.default\_hooks](mini-doc.qmd#minidoc.default_hooks).
...
- <span class="help-syntax-special">{aliases}</span> - table with keys being alias name and values - alias
  description and single string (using `\n` to separate lines).

- <span class="help-syntax-special">{eval_section}</span> - input section of `@eval` section hook. Can be used for
  information about current block, etc.

- <span class="help-syntax-special">{toc}</span> - array with table of contents entries. Each entry is a whole
  `@toc_entry` section.
...
#### Algorithm {#minidoc.generate-algorithm}
...
#### Project specific script {#minidoc.generate-projectspecificscript}
...
#### Parameters {#minidoc.generate-parameters}



<span class="help-syntax-special">{input}</span> `(table|nil)` Array of file paths which will be processed in supplied
...
<span class="help-syntax-special">{output}</span> `(string|nil)` Path for output help file. Default:
  `doc/<current-directory>.txt` (designed to be used for generating help
  file for plugin).

<span class="help-syntax-special">{config}</span> `(table|nil)` Configuration overriding parts of [MiniDoc.config](mini-doc.qmd#minidoc.config).

#### Return {#minidoc.generate-return}
...
#### Directives {#minidoc.afterlines_to_code-directives}
...
#### Parameters {#minidoc.afterlines_to_code-parameters}



<span class="help-syntax-special">{struct}</span> `(table)` Block or section structure which after lines will be
  converted to code.

#### Return {#minidoc.afterlines_to_code-return}
