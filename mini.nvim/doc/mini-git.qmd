...
- Automated tracking of Git related data: root path, status, HEAD, etc.
  ...
  See [`MiniGit.enable()`](mini-git.qmd#minigit.enable) and [`MiniGit.get_buf_data()`](mini-git.qmd#minigit.get_buf_data) for more information.
...
- Helper functions to inspect Git history:
    ...
    - [`MiniGit.show_range_history()`](mini-git.qmd#minigit.show_range_history) shows how certain line range evolved.

    - [`MiniGit.show_diff_source()`](mini-git.qmd#minigit.show_diff_source) shows file state as it was at diff entry.

    - [`MiniGit.show_at_cursor()`](mini-git.qmd#minigit.show_at_cursor) shows Git related data depending on context.
...
- [`MiniGit.get_buf_data()`](mini-git.qmd#minigit.get_buf_data)
...
#### Comparisons {#minigit-comparisons}
...
Function [`MiniGit.show_at_cursor()`](mini-git.qmd#minigit.show_at_cursor) is specifically exported to make Git
...
- Call inside buffer for already committed file to show the evolution of
  ...
  This also works inside output of [`MiniGit.show_diff_source()`](mini-git.qmd#minigit.show_diff_source).
...
- `MiniGitCommandDone` - after command is done executing. For Lua callbacks it
  provides a special `data` table with the following fields:
    ...
    - <span class="help-syntax-keys">\<cmd_input\></span> `(table)` - structured data about executed command.
      Has same structure as Lua function input in [`nvim_create_user_command()`](https://neovim.io/doc/user/helptag.html?tag=nvim_create_user_command\(\)).
...
### show_at_cursor() {#minigit.show_at_cursor}
...
- If there is a commit-like [`<cword>`](https://neovim.io/doc/user/helptag.html?tag=<cword>), show it in split.

- If possible, show diff source via [`MiniGit.show_diff_source()`](mini-git.qmd#minigit.show_diff_source).

- If possible, show range history via [`MiniGit.show_range_history()`](mini-git.qmd#minigit.show_range_history).
...
### show_diff_source() {#minigit.show_diff_source}
...
`:Git log --patch`, `:Git diff`, or [`MiniGit.show_range_history()`](mini-git.qmd#minigit.show_range_history)),
...
### show_range_history() {#minigit.show_range_history}
...
- Works well with [`MiniGit.diff_foldexpr()`](mini-git.qmd#minigit.diff_foldexpr).
...
### diff_foldexpr() {#minigit.diff_foldexpr}
...
Works well with [`MiniGit.show_range_history()`](mini-git.qmd#minigit.show_range_history).
...
- [`MiniGit.get_buf_data()`](mini-git.qmd#minigit.get_buf_data). See its help for a list of actually tracked data.
...
- `vim.b.minigit_summary` (table) and `vim.b.minigit_summary_string` (string)
  ...
  and in progress action (see [`MiniGit.get_buf_data()`](mini-git.qmd#minigit.get_buf_data) for more details).
...
Note that not all data listed in [`MiniGit.get_buf_data()`](mini-git.qmd#minigit.get_buf_data) can be present (yet)
...
### get_buf_data() {#minigit.get_buf_data}
