...
- If you experience flicker when typing near highlighted pattern in Insert
  ...
  For better experience with [mini.completion](mini-completion.qmd#mini.completion), make sure that its
...
- Globally with `require('mini.hipatterns').setup({})` (replace `{}` with
  ...
  buffers (see ['buftype'](https://neovim.io/doc/user/helptag.html?tag='buftype')). Use [MiniHipatterns.enable()](mini-hipatterns.qmd#minihipatterns.enable) to manually enable
  in other buffers.
...
<a name="minihipatterns-hl-groups" href="mini-hipatterns.qmd#minihipatterns-hl-groups"><b>MiniHipatterns-hl-groups</b></a>
...
<p align="center">`'%f[%w]()TODO()%f[%W]'`. In this case, for example, `TODOING` or `MYTODO`</p>
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Options. Possible fields:
  ...
  - <span class="help-syntax-keys">\<max_number\></span> `(number)` - as in [MiniHipatterns.compute\_hex\_color\_group()](mini-hipatterns.qmd#minihipatterns.compute_hex_color_group).
...
<p align="center">`MiniHipatterns.compute_hex_color_group`(<span class="help-syntax-special">{hex_color}</span>, <span class="help-syntax-special">{style}</span>, <span class="help-syntax-special">{opts}</span>)</p>
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Options. Possible fields:
  ...
  - <span class="help-syntax-keys">\<max_number\></span> `(number)` - maximum number of different highlight groups
    this function is allowed to create. Useful to avoid [E849](https://neovim.io/doc/user/helptag.html?tag=E849).
    Default: 10000.
