...
- Comment structure by default is inferred from ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring'): either
...
- To use tree-sitter aware commenting, global value of ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring')
...
`options.custom_commentstring` can be a function customizing ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring')
option used to infer comment structure. It is called once before every
commenting action with the following arguments:

- `ref_position` - position at which to compute ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring') (might be
...
Its output should be a valid ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring') (string containing `%s`).

If not set or the output is `nil`, [MiniComment.get\_commentstring()](mini-comment.qmd#minicomment.get_commentstring) is used.

For example, this option can be used to always use buffer ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring')
...
- Changing ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring') in `hooks.pre` is allowed and will take effect.
...
<span class="help-syntax-special">{mode}</span> `(string|nil)` Optional string with ['operatorfunc'](https://neovim.io/doc/user/helptag.html?tag='operatorfunc') mode (see [g@](https://neovim.io/doc/user/helptag.html?tag=g@)).
...
- Comment structure is inferred from buffer's ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring') option or
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Options. Possible fields:
  ...
  - <span class="help-syntax-keys">\<ref_position\></span> `(table)` - A two-value array with `{ row, col }` (both
    starting at 1) of reference position at which ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring') value
...
Get ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring')

This represents default approach of computing relevant ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring')
option in current buffer. Used to infer comment structure.

It has the following logic:

- If there is an active tree-sitter parser, try to get ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring') from
  the local language at `ref_position`.


- If first step is not successful, use buffer's ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring') directly.
...
<span class="help-syntax-special">{ref_position}</span> `(table)` Reference position inside current buffer at which
  to compute ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring'). Same structure as `opts.ref_position`
  in [MiniComment.toggle\_lines()](mini-comment.qmd#minicomment.toggle_lines).

#### Return {#minicomment.get_commentstring-return}


`(string)` Relevant value of ['commentstring'](https://neovim.io/doc/user/helptag.html?tag='commentstring').
