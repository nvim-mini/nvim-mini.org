...
#### Disabling {#minicomment-disabling}
...
- `ref_position` - position at which to compute 'commentstring' (might be
  ...
  is the same as `opts.ref_position` in [`MiniComment.toggle_lines()`](mini-comment.qmd#minicomment.toggle_lines).

Its output should be a valid 'commentstring' (string containing `%s`).

If not set or the output is `nil`, [`MiniComment.get_commentstring()`](mini-comment.qmd#minicomment.get_commentstring) is used.
...
### operator() {#minicomment.operator}

<p align="center">`MiniComment.operator`(<span class="help-syntax-special">{mode}</span>)</p>
Main function to be mapped

It is meant to be used in expression mappings (see [`:map-<expr>`](https://neovim.io/doc/user/helptag.html?tag=:map-<expr>)) to enable
...
### toggle_lines() {#minicomment.toggle_lines}
...
### textobject() {#minicomment.textobject}
...
### get_commentstring() {#minicomment.get_commentstring}
...
<span class="help-syntax-special">{ref_position}</span> `(table)` Reference position inside current buffer at which
  ...
  in [`MiniComment.toggle_lines()`](mini-comment.qmd#minicomment.toggle_lines).
