...
- [`MiniClue.gen_clues`](mini-clue.qmd#miniclue.gen_clues).
...
- Triggers are implemented as special buffer-local mappings. This leads to
  several caveats:
    ...
    - They need to be the latest created buffer-local mappings or they will
      ...
      run either [`MiniClue.setup()`](mini-clue.qmd#miniclue.setup) or [`MiniClue.ensure_buf_triggers()`](mini-clue.qmd#miniclue.ensure_buf_triggers).
...
- Descriptions from existing mappings take precedence over user-supplied
  ...
  mapping, use [`MiniClue.set_mapping_desc()`](mini-clue.qmd#miniclue.set_mapping_desc).
...
- Due to technical difficulties, there is no foolproof support for
  Operator-pending mode triggers (like `a`/`i` from [`mini.ai`](mini-ai.qmd#mini.ai)):
    ...
    - Doesn't work as part of a command in "temporary Normal mode" (like
      after [`i_CTRL-O`](https://neovim.io/doc/user/helptag.html?tag=i_CTRL-O)) due to implementation difficulties.
...
#### Comparisons {#miniclue-comparisons}
...
#### Highlight groups {#miniclue-highlightgroups}
...
### Key query process {#miniclue-key-query-process}
...
- Wait (indefinitely) for user to press a key. Advance depending on the key:
    ...
    - Special key:
        ...
        - If `<CR>`, stop the process and execute current key query, meaning
          emulate (with [`nvim_feedkeys()`](https://neovim.io/doc/user/helptag.html?tag=nvim_feedkeys\(\))) user pressing those keys.
...
this module plus all [`<Leader>`](https://neovim.io/doc/user/helptag.html?tag=<Leader>) mappings in Normal and Visual modes:
...
Assume there are these [`<Leader>`](https://neovim.io/doc/user/helptag.html?tag=<Leader>) mappings set up:
...
The following setup will enable [`<Leader>`](https://neovim.io/doc/user/helptag.html?tag=<Leader>) as trigger in Normal and Visual
...
a subset of built-in completion (see [`MiniClue.gen_clues.builtin_completion()`](mini-clue.qmd#miniclue.gen_clues.builtin_completion)
...
special non-listed buffers. Use [`MiniClue.ensure_buf_triggers()`](mini-clue.qmd#miniclue.ensure_buf_triggers) to manually
...
- Submode for window commands using [`MiniClue.gen_clues.windows()`](mini-clue.qmd#miniclue.gen_clues.windows):
...
- To use [`<Leader>`](https://neovim.io/doc/user/helptag.html?tag=<Leader>) as part of the config (either as trigger or inside clues),
...
- Postkeys are literal simulation of keypresses with [`nvim_feedkeys()`](https://neovim.io/doc/user/helptag.html?tag=nvim_feedkeys\(\)).
...
structure as in [`nvim_open_win()`](https://neovim.io/doc/user/helptag.html?tag=nvim_open_win\(\)) with the following enhancements:
...
- <span class="help-syntax-keys">\<row\></span> and <span class="help-syntax-keys">\<col\></span> can be equal to `"auto"` in which case they will be
  computed to "stick" to set anchor ("SE" by default; see [`nvim_open_win()`](https://neovim.io/doc/user/helptag.html?tag=nvim_open_win\(\))).
...
### enable_all_triggers() {#miniclue.enable_all_triggers}
...
### enable_buf_triggers() {#miniclue.enable_buf_triggers}
...
### disable_all_triggers() {#miniclue.disable_all_triggers}
...
### disable_buf_triggers() {#miniclue.disable_buf_triggers}
...
### ensure_all_triggers() {#miniclue.ensure_all_triggers}
...
### ensure_buf_triggers() {#miniclue.ensure_buf_triggers}
...
### set_mapping_desc() {#miniclue.set_mapping_desc}
...
### gen_clues.builtin_completion() {#miniclue.gen_clues.builtin_completion}
...
### gen_clues.g() {#miniclue.gen_clues.g}
...
### gen_clues.square_brackets() {#miniclue.gen_clues.square_brackets}
...
### gen_clues.marks() {#miniclue.gen_clues.marks}
...
Note: if you use "g" as trigger (like to enable [`MiniClue.gen_clues.g()`](mini-clue.qmd#miniclue.gen_clues.g)),
...
### gen_clues.registers() {#miniclue.gen_clues.registers}
...
### gen_clues.windows() {#miniclue.gen_clues.windows}
...
### gen_clues.z() {#miniclue.gen_clues.z}
