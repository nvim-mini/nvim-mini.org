...
- Manipulate files and directories by editing text buffers: create, delete,
  ...
  See [MiniFiles-manipulation](mini-files.qmd#minifiles-manipulation) for an overview.
...
<a name="minifiles-hl-groups" href="mini-files.qmd#minifiles-hl-groups"><b>MiniFiles-hl-groups</b></a>
...
#### LSP integration {#minifiles-manipulation-lspintegration}
...
It can be disabled by setting `options.lsp_timeout = 0` in [MiniFiles.config](mini-files.qmd#minifiles.config).
...
- `MiniFilesWindowOpen` - when new window is opened. Can be used to set
  window-local settings (like border, ['winblend'](https://neovim.io/doc/user/helptag.html?tag='winblend'), etc.)
...
LSP integration requests (see [MiniFiles-manipulation](mini-files.qmd#minifiles-manipulation)).
...
<span class="help-syntax-special">{opts}</span> `(table|nil)` Table of options overriding local options of active
  explorer session.
