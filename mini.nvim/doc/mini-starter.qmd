...
- Modify the final look by supplying content hooks: functions which take
  buffer content (see [MiniStarter.get\_content()](mini-starter.qmd#ministarter.get_content)) and identifier as input
  while returning buffer content as output. There are pre-configured
  content hook generators in [MiniStarter.gen\_hook](mini-starter.qmd#ministarter.gen_hook).
...
<a name="ministarter-hl-groups" href="mini-starter.qmd#ministarter-hl-groups"><b>MiniStarter-hl-groups</b></a>
...
- `MiniStarterItemBullet` - units from [MiniStarter.gen\_hook.adding\_bullet()](mini-starter.qmd#ministarter.gen_hook.adding_bullet).
...
### Example config {#ministarter-example-config .help-syntax-right-anchor}
...
### Lifecycle {#ministarter-lifecycle .help-syntax-right-anchor}
...
- Wait for user to choose an item. This is done using following logic:
    ...
    - Typing any character from `MiniStarter.config.query_updaters` leads
      to updating query. Read more in [MiniStarter.add\_to\_query()](mini-starter.qmd#ministarter.add_to_query).
...
- Set buffer mappings. Besides basic mappings (described inside "Lifecycle
  ...
  [MiniStarter.add\_to\_query()](mini-starter.qmd#ministarter.add_to_query).
...
- Make initial buffer content (see [MiniStarter.get\_content()](mini-starter.qmd#ministarter.get_content) for a
...
- Gather final items from content with [MiniStarter.content\_to\_items()](mini-starter.qmd#ministarter.content_to_items).

- Convert content to buffer lines with [MiniStarter.content\_to\_lines()](mini-starter.qmd#ministarter.content_to_lines) and
  add them to buffer.
...
- All actions require [mini.pick](mini-pick.qmd#mini.pick) module of [mini.nvim](mini-nvim.qmd#mini.nvim).

- "Command history", "Explorer", and "Visited paths" items
  require [mini.extra](mini-extra.qmd#mini.extra) module of [mini.nvim](mini-nvim.qmd#mini.nvim).

- "Visited paths" items requires [mini.visits](mini-visits.qmd#mini.visits) module of [mini.nvim](mini-nvim.qmd#mini.nvim).
...
Basically, this computes left and top pads for [MiniStarter.gen\_hook.padding()](mini-starter.qmd#ministarter.gen_hook.padding)
...
See [MiniStarter.content\_to\_lines()](mini-starter.qmd#ministarter.content_to_lines) for converting content to buffer lines
and [MiniStarter.content\_to\_items()](mini-starter.qmd#ministarter.content_to_items) - to list of parsed items.
...
- Computes some helper information about how item will be actually
  displayed (after [MiniStarter.content\_to\_lines()](mini-starter.qmd#ministarter.content_to_lines)) and minimum number of
...
- Modifies item's `name` element taking it from corresponding `string`
  ...
  of content hooks (like, for example, in [MiniStarter.gen\_hook.indexing()](mini-starter.qmd#ministarter.gen_hook.indexing)).
