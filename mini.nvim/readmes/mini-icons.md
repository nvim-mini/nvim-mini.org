...
See more details in [Features](#features) and [Documentation](../doc/mini-icons.qmd).
...
- Provide icons with their highlighting via a single [`MiniIcons.get()`](../doc/mini-icons.qmd#miniicons.get) for various categories: filetype, file/directory path, extension, operating system, LSP kind values. Icons and category defaults can be overridden.
...
- Mocking methods of 'nvim-tree/nvim-web-devicons' for better integrations with plugins outside 'mini.nvim'. See [`:h MiniIcons.mock_nvim_web_devicons()`](../doc/mini-icons.qmd#miniicons.mock_nvim_web_devicons).

- Tweaking built-in maps for "LSP kind" to include icons. In particular, this makes ['mini.completion'](https://github.com/nvim-mini/mini.nvim/blob/main/readmes/mini-completion.md) use icons in LSP step. See [`:h MiniIcons.tweak_lsp_kind()`](../doc/mini-icons.qmd#miniicons.tweak_lsp_kind).

Notes:

- It is not a goal to become a collection of icons for as much use cases as possible. There are specific criteria for icon data to be included as built-in in each category (see [`:h MiniIcons.get()`](../doc/mini-icons.qmd#miniicons.get)). The main supported category is "filetype".
...
- Use [`MiniIcons.get()`](../doc/mini-icons.qmd#miniicons.get) function to get icon string and more data about it.

- For file icons prefer using full path instead of relative or only basename. It makes a difference if path matches pattern that uses parent directories. The [`:h MiniIcons.config`](../doc/mini-icons.qmd#miniicons.config) has an example of that.
