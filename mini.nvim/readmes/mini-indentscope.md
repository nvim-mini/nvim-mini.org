...
See more details in [Features](#features) and [Documentation](../doc/mini-indentscope.qmd).
...
- Customization of scope computation options can be done on global level (in [`MiniIndentscope.config`](../doc/mini-indentscope.qmd#miniindentscope.config)), for a certain buffer (using `vim.b.miniindentscope_config` buffer variable), or within a call (using `opts` variable in [`MiniIndentscope.get_scope()`](../doc/mini-indentscope.qmd#miniindentscope.get_scope)).
