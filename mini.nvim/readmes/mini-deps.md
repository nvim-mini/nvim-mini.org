...
- On Neovim>=0.12 it is recommended to use [vim.pack](https://neovim.io/doc/user/helptag.html?tag=vim.pack) for plugin management and [`:h MiniMisc.safely()`](../doc/mini-misc.qmd#minimisc.safely) as [`MiniDeps.now()`](../doc/mini-deps.qmd#minideps.now) and [`MiniDeps.later()`](../doc/mini-deps.qmd#minideps.later) alternative. See [https://echasnovski.com/vim-pack-guide](https://echasnovski.com/vim-pack-guide) for more details, including recommendations about migration from 'mini.deps'.
...
- Update corresponding [`MiniDeps.add()`](../doc/mini-deps.qmd#minideps.add) call.
