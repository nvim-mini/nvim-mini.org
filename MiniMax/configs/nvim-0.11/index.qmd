...
-- Example usage:
-- - `MiniDeps.add('...')` - add inside config to add a plugin.
...
-- Example usage:
-- - `ghip` - apply hunks (`gh`) within *i*nside paragraph
...
--
-- Basic manipulation (start in Normal mode):
-- - New entry: press `o` and type its name; end with `/` to create directory
-- - Rename: press `C` and type new name
-- - Delete: press `dd`
-- - Move/copy: press `dd`/`yy`, navigate to target directory, press `p`
-- - After any action, press `=` to synchronize, read carefully about actions,
--   press `y` or `<CR>` to confirm
--
-- See also:
...
later(function()
  ...
  -- - `'c` to navigate your config directory
...
-- - `<Leader>gd` - show unstaged changes as a patch in separate tabpage
-- - `<Leader>gL` - show Git log of current file
-- - `:Git help git` - show output of `git help help` inside Neovim
--
-- See also:
...
-- Example usage:
-- - `:Pick hipatterns` - pick among all higlighted patterns
--
-- See also:
...
later(function()
  ...
  hipatterns.setup({
    highlighters = {
      ...
      -- Hightlight hex color string (#aabbcc) with that color as a background
...
-- with animated vertical line. Provides relevatn motions and textobjects.
-- Example usage:
...
--   should be reduced
-- - Keep typing until target spot is unique to perform the jump
--
-- See also:
...
-- - `<Leader>mt` - toggle map window (closed by default)
-- - `<Leader>mf` - focus on the map for fast navigation
-- - `<Leader>ms` - change map's side (if it covers something underneath)
--
-- See also:
...
later(function()
  ...
  -- Map built-in navigtaion characters to force map refresh.
  for _, key in ipairs({ 'n', 'N', '*', '#' }) do
    local rhs = key
      -- Also open enough folds when jumping to next match
      .. 'zv'
...
-- - `:lua MiniMisc.stat_summary(MiniMisc.bench_time(f, 100))` - run function `f`
--   100 times and report statistical summary of execution times.
later(function()
  -- Makes `:h MiniMisc.put()` and `:h MiniMisc.put_text()` public
  require('mini.misc').setup()

  -- Make current working directory change based on the current file path.
  -- It searches up the file tree until first root marker ('.git' or 'Makefile')
  ...
  -- possibly different color padding outside of Neovim instance
...
-- right character if it is already to the right. Also provides mappings for `<CR>`
-- and `<BS>` to perform extra actions when inside pair.
...
--   At the top here is a current query used to filter+sort items.
...
-- - Press `<Tab>` to show item's preview. Press `<Tab>` again to go back to items.
...
--   config config or plugins, or inside `setup()` call directly.
-- - This config, however, does come with snippets:
...
-- - If you don't remember snippet's prefix, type only part of its prefix (or none
--   at all) and press `<C-j>`. It should show picker with all snippets that have
--   prefixes matching typed characters (or all snippets if none was typed).
...
later(function()
  ...
  snippets.setup({
    snippets = {
      ...
      -- Load from 'snippets/' directory of plugins
...
-- The module comes with many built-in surroundings, each identified by
-- a single character. It searches only for surrounding that covers cursor and
-- comes with special "next" / "last" versions of actions to search forward or
-- backward (just like 'mini.ai').
...
-- tracked on disk to later reuse: show in special frecency order, etc.
-- It also supports adding labels to visits to quickly navigate between them.
-- Example usage:
-- - `<Leader>fn` - find across all visits
...
--
-- This config is designed for using Lua with Neovim. It provides only basic
-- config and can be further improved.
return {
